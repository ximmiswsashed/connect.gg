...
async function lockGameMouse() {
  ...
  } catch (_) { rdpConnStatus.textContent = 'Mouse lock unavailable. Allow mouse lock in your browser, then click Game mouse again.'; }
}
document.addEventListener('pointerlockchange', () => {
  clearHeldInput();
  $('rdp-lock-btn').textContent = mouseLocked() ? 'Unlock · Esc' : 'Game mouse';
  if (mouseLocked()) streamFeed.focus({preventScroll:true});
});
document.addEventListener('pointerlockerror', () => {
  rdpConnStatus.textContent = 'Mouse lock was denied by the browser. Click Game mouse to retry.';
});
