...
loginForm.addEventListener('submit', event => {
  ...
  if (pairingCode.length < 16) return showError('Enter your home PC pairing code (at least 16 characters).');
  try {
    const url = new URL(bridgeUrlIn.value.trim());
    if (url.protocol !== 'https:' || url.username || url.password) throw new Error();
    bridgeUrl = url.origin;
  } catch (_) {
    return showError('Paste the current https://…trycloudflare.com bridge address.');
  }
  pairingCodeIn.value = '';
...
function handleDesktopClick(num) { openDesktop(num); }
...
async function bridgeFetch(path, body, base = bridgeUrl) {
  ...
  try {
    ...
    if (error.name === 'AbortError' || error instanceof TypeError) {
      throw new Error('Cannot reach the home bridge. Keep both PowerShell windows running and check the tunnel address.');
    }
    throw error;
...
async function openDesktop(num) {
  ...
  try {
    const result = await bridgeFetch('/control/pair', { pairingCode, desktop: num }, base);
    if (current !== epoch) {
  ...
  } catch (error) { if (current === epoch) failDesktop(error.message); }
}
...
function connectSockets(current) {
  ...
  input.onerror = () => { if (current === epoch) failDesktop('Control connection failed. Check the tunnel, then reopen the desktop.'); };
  input.onclose = () => { if (current === epoch) failDesktop('Control connection closed. Reopen the desktop to reconnect.'); };
  const start = performance.now();
  watchdog = setInterval(() => {
    if (current !== epoch) return;
    const now = performance.now();
    if (!hasFrame && now - start > 15000) failDesktop('No frames received. Check the home bridge window for an error.');
    else if (hasFrame && now - lastFrameAt > 8000) failDesktop('Video stopped arriving. Reopen the desktop to reconnect.');
    else if (controlReady && now - lastPongAt > 8000) failDesktop('Control connection timed out. Reopen the desktop to reconnect.');
    else updateStats();
  }, 1000);
}
...
function startVideo(current) {
  ...
  socket.onmessage = event => {
    ...
    if (typeof event.data === 'string') {
      try {
        const message = JSON.parse(event.data);
        if (message.type === 'error') failDesktop(message.message);
        else if (message.type === 'idle') lastFrameAt = performance.now();
      } catch (_) { failDesktop('Invalid video response from the home bridge.'); }
      return;
    }
    const data = event.data;
    if (data.byteLength <= 28) return failDesktop('Invalid video frame.');
  ...
  socket.onerror = () => { if (current === epoch) failDesktop('Video connection failed. Check the home tunnel and reconnect.'); };
  socket.onclose = () => { if (current === epoch) failDesktop('Video connection closed. Reopen the desktop to reconnect.'); };
}
  ...
  try {
    while (waitingFrame) {
      ...
      try {
        ...
        if (frame.current !== epoch) continue;
        ...
        if (frame.current === epoch) failDesktop('Could not decode the screen frame. Use a current Chrome, Edge, Firefox or Safari browser.');
      } finally {
        bitmap?.close();
        ackFrame(frame);
      }
...
function imagePoint(event, clamp = false) {
  if (!hasFrame || !sourceGeometry) return null;
...
document.addEventListener('keydown', event => {
  if (!controlReady || !hasFrame || document.activeElement !== streamFeed) return;
...
function stopConnections() {
  ...
  clearTimeout(moveTimer); clearTimeout(pointerTimer);
  heartbeat = watchdog = moveTimer = pointerTimer = null;
  pendingInput = [];
