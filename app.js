...
function connectWebRTC(desktopNum) {
  ...
  viewerPeer.on('open', () => {
    ...
    controlConnection.on('data', (message) => handleControlMessage(message, desktopNum, epoch));
    controlConnection.on('close', () => {
      if (epoch !== connectionEpoch) return;
      controlReady = false;
      failOrRetry('Control channel disconnected — retrying…', epoch);
    });
    controlConnection.on('error', () => failOrRetry('Control channel error — retrying…', epoch));
  });

  viewerPeer.on('error', (error) => {
    if (epoch !== connectionEpoch) return;
    if (error.type === 'peer-unavailable') {
      failOrRetry('Home broadcaster is offline. Make sure it is running.', epoch);
    } else {
      failOrRetry(`Connection error: ${error.type || 'unknown'}`, epoch);
    }
  });
  viewerPeer.on('disconnected', () => {
    if (epoch === connectionEpoch) failOrRetry('Signaling connection dropped — retrying…', epoch);
  });
}

function handleControlMessage(message, desktopNum, epoch) {
  if (epoch !== connectionEpoch) return;
  ...
  activeCall.on('close', () => failOrRetry('Broadcaster disconnected — retrying…', epoch));
  activeCall.on('error', () => failOrRetry('Video connection error — retrying…', epoch));
}

function failOrRetry(message, epoch = connectionEpoch) {
  if (epoch !== connectionEpoch || retryTimer) return;
  controlReady = false;
  ...
  retryTimer = setTimeout(() => {
    retryTimer = null;
    if (rdpOverlay.classList.contains('active') && epoch === connectionEpoch) connectWebRTC(currentDesktopNum);
  }, 3500);
}
...
function queuePointerMove(event) {
  ...
  // Relative input avoids coordinate/DPI mismatches between the two computers.
  ...
  requestAnimationFrame(() => {
    ...
    if (x || y) sendControl({ type: 'input', kind: 'pointer', action: 'move-relative', deltaX: x, deltaY: y });
  });
}
...
streamFeed.addEventListener('pointerdown', (event) => {
  ...
  sendControl({ type: 'input', kind: 'pointer', action: 'down', button: event.button });
});
streamFeed.addEventListener('pointerup', (event) => {
  event.preventDefault();
  sendControl({ type: 'input', kind: 'pointer', action: 'up', button: event.button });
});
streamFeed.addEventListener('contextmenu', (event) => event.preventDefault());
streamFeed.addEventListener('wheel', (event) => {
  event.preventDefault();
  sendControl({ type: 'input', kind: 'pointer', action: 'wheel', deltaX: event.deltaX, deltaY: event.deltaY });
}, { passive: false });
