...
function startPreferredVideo(current, pairingResult) {
  ...
  try {
    mediaReader = new MediaMTXWebRTCReader({
      ...
      onError: () => { if (!receivedTrack && current === epoch) startJpegFallback(current); },
      onTrack: event => {
        if (current !== epoch || event.track.kind !== 'video') return;
        receivedTrack = true;
        obsFeed.srcObject = event.streams[0];
        obsFeed.play().catch(()=>{});
      }
    });
    obsFeed.onplaying = () => {
      ...
      streamFeed.width=1920; streamFeed.height=1080;
      ...
      setOverlayState('live'); rdpConnStatus.textContent='Live · OBS low latency';
      streamFeed.focus({preventScroll:true}); watchObsFrames(current);
    };
    mediaFallbackTimer=setTimeout(()=>{if(current===epoch&&!hasFrame)startJpegFallback(current);},8000);
...
function startJpegFallback(current) {
  ...
  rdpConnStatus.textContent='OBS path unavailable · using direct fallback';
  startVideo(current);
}
...
function updateStats() {
  ...
  rdpConnStatus.textContent = (mediaMode==='obs'?'OBS · ':'Fallback · ') + resolution + ' · ' + fps +
    '/60 FPS · ' + (rttMs === null ? '…' : Math.round(rttMs)) + ' ms RTT' + (mediaMode==='jpeg'?' · '+mbps+' Mbps':'');
...
function sendControl(message) {
  ...
  if (message.action === 'move-relative' && pendingInput.at(-1)?.action === 'move-relative') {
    pendingInput.at(-1).deltaX += message.deltaX;
    pendingInput.at(-1).deltaY += message.deltaY;
...
document.addEventListener('pointerlockchange', () => {
  ...
  if (mouseLocked()) streamFeed.focus({preventScroll:true});
});
...
document.addEventListener('mousemove', event => {
  ...
  if (pendingMove?.action !== 'move-relative') pendingMove = {kind:'pointer',action:'move-relative',deltaX:0,deltaY:0};
  pendingMove.deltaX += event.movementX;
  pendingMove.deltaY += event.movementY;
  if (performance.now() - lastSentMove >= 8) flushPointer();
  else if (!pointerTimer) pointerTimer = setTimeout(() => {pointerTimer=null;flushPointer();},8);
});
...
function stopConnections() {
  ...
  obsFrameCallback=null;mediaMode=null;obsFeed.pause();obsFeed.srcObject=null;obsFeed.style.display='none';
